...
![Frame 2147238197](/一种py注入的ctf解题思路/Frame%202147238197-20260131134535-h6ttp05.png)
...
1. 将`目标exe`和`code.py`两个文件放在同一目录下。
    ...
    ![9e691a0154f6706e](/一种py注入的ctf解题思路/9e691a0154f6706e-20260131140100-07s7sva.png)
2. 在`code.py`中编写脚本  
    如
...
4. 使用[Process Hacker 2](https://github.com/bigrayhicks/processhacker2) (一个魔改的[systeminformer](https://github.com/winsiderss/systeminformer))进行dll注入
    ...
    ![image](/一种py注入的ctf解题思路/image-20260131140256-53ah7js.png)

    找到刚刚解压的pyInjector的dll，根据系统选择注入的dll

    ![be648b8f22acff2a](/一种py注入的ctf解题思路/be648b8f22acff2a-20260131140359-tgu79nb.png)

‍

可以看到注入的瞬间执行了`code.py`​

![e8b5a054098301dd](/一种py注入的ctf解题思路/e8b5a054098301dd-20260131140517-gz0sqnc.png)
...
1. 打印`全局变量`​
    ...
    ![88719a3512658a9a](/一种py注入的ctf解题思路/88719a3512658a9a-20260131140829-plgyk0q.png)
    ...
    ![cc8530e0d684e729](/一种py注入的ctf解题思路/cc8530e0d684e729-20260131141046-egh0pyv.png)
...
2. 如果是`Nuitka`，我们不能dis出代码
    ...
    ![9167b9d0fd8ad9a9](/一种py注入的ctf解题思路/9167b9d0fd8ad9a9-20260131141651-h3lb6q4.png)
